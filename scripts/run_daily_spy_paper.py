...
def serialize_result(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    serialized = {
        ...
        "trade_ideas": [
            _serialize_datetime(idea.model_dump()) if hasattr(idea, "model_dump") else str(idea)
            for idea in result["trade_ideas"]
        ],
        "order_results": [
            _serialize_datetime(order.model_dump()) if hasattr(order, "model_dump") else str(order)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="simulated",
        help="Broker to use: simulated (default) or alpaca (requires ALPACA_API_KEY env)",
    )
    ...
    if args.broker == "alpaca":
        print("Using Alpaca broker (paper trading)...")
        try:
        ...
        print("Using simulated broker...")
    
    components = build_default_pipeline_components(broker=broker)
